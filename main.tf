...
resource "aws_vpc_endpoint_policy" "vpc_endpoint_policy" {
  ...
  vpc_endpoint_id = element(aws_vpc_endpoint.vpc_endpoint.*.id, 0)
...
resource "aws_api_gateway_rest_api" "api_gateway_rest" {
  ...
  endpoint_configuration {
    types            = ["PRIVATE"]
    vpc_endpoint_ids = [one(aws_vpc_endpoint.vpc_endpoint.*.id)]
  }
...
resource "aws_api_gateway_api_key" "api_key" {
  ...
  name        = "api-key-${var.apigateway_name}-${var.environment}-daviplata"
  description = "Api key ${var.apigateway_name} (${var.environment}"
  enabled     = true
...
resource "aws_api_gateway_usage_plan" "api-usage-plan" {
  count = var.create_api_key ? 1 : 0
  name  = "usage-plan-${var.apigateway_name}-${var.environment}"
...
resource "aws_api_gateway_usage_plan_key" "usage_plan_key" {
  ...
  key_type      = "API_KEY"
  usage_plan_id = aws_api_gateway_usage_plan.api-usage-plan[0].id
}
